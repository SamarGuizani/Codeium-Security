...
{
    // Test de diagnostic temporaire (harnais uniquement) : reutilise le cache OCR deja
    // ecrit par RestBatchDiagnosticTest (dossier Desktop\Rest) pour lister, pour chaque
    // fichier, le CustomerName et l'ExtractionPeriod detectes par GenericDocumentMetadataExtractor.
    // N'appelle jamais BankDocumentParser : ne verifie/ne touche rien aux transactions.
    [Collection("Pipeline")]
    public class MetadataBatchDiagnosticTest
    {
        private readonly PipelineFixture _fixture;
        private readonly ITestOutputHelper _output;

        public MetadataBatchDiagnosticTest(PipelineFixture fixture, ITestOutputHelper output)
        {
            _fixture = fixture;
            _output = output;
        }
