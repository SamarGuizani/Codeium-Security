...
{
    [Collection("Pipeline")]
    public class AbcInvestigateTest2
    {
        private readonly PipelineFixture _fixture;
        private readonly ITestOutputHelper _output;

        public AbcInvestigateTest2(PipelineFixture fixture, ITestOutputHelper output)
        {
            _fixture = fixture;
            _output = output;
        }
        ...
        {
            string pdfPath = "C:\\Users\\USER\\OneDrive\\Desktop\\codeuim\\ABC\\RLV ABC.pdf";
            Assert.True(File.Exists(pdfPath), $"Introuvable: {pdfPath}");

            var result = await _fixture.ProcessingService.ProcessFileAsync(pdfPath, "RLV ABC.pdf");
